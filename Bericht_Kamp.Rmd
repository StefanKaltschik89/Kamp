...
```{r Datensatz mergen}
kamp<-rbind(revier_rosenburg, Stiefern)
```



```{r Auswertung, message=FALSE, warning=FALSE}
df <- revier_rosenburg %>% 
    group_by(Datum, Ort) %>% 
...
plot<-ggplot(kamp, aes(x = Datum, y = Temperature)) + 
    geom_smooth(aes(color = Ort), size = 1, se=F) +
    scale_color_manual(values = c("#00AFBB", "#E7B800", "green")) +
    theme_minimal()

plot
# Set axis limits c(min, max)
# min <- as.Date("2022-07-10")
...
Statistik<-revier_rosenburg %>% group_by(Ort) %>% summarise(min(Temperature), max(Temperature), mean(Temperature), var(Temperature), median(Temperature))
